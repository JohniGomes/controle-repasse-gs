...
async function loadMetas() {
  try {
    const res = await apiCall({ action: 'getMetas' });
    metasData = res.data || [];
    renderMetasTabela();
  } catch { /* silencioso */ }
}
...
function renderMetasTabela() {
  ...
  tbody.innerHTML = sorted.map(m => `
    <tr>
      <td style="font-weight:600">${formatMes(m.mes)}</td>
      <td>${m.dentista}</td>
      <td>${m.meta || '—'}</td>
      <td>${m.indicacoes || '—'}</td>
      ...
      <td class="td-actions">
        ...
        <button class="btn-action btn-del" onclick="deletarMeta('${m.id}')" title="Excluir">
...
function populateMetaDentistas() {
  const sel = document.getElementById('metaDentista');
  const cur = sel.value;
  sel.innerHTML = '<option value="">Selecione...</option>';
  dentistas.forEach(d => {
    const o = document.createElement('option');
...
async function salvarMeta() {
  ...
  finally { btn.disabled = false; btn.innerHTML = '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="20 6 9 17 4 12"/></svg> Salvar Meta'; }
}

async function deletarMeta(id) {
  if (!confirm('Excluir este registro de meta?')) return;
  try {
    const res = await apiCall({ action: 'deleteMeta', id });
    if (res.error) { showToast(res.error, 'error'); return; }
...
// Carrega metas junto com os outros dados
document.addEventListener('DOMContentLoaded', async () => {
  await loadMetas();
  populateMetaDentistas();
  ...
  const now = new Date();
  document.getElementById('metaMes').value =
